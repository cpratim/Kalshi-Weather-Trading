...
def start_algorithm(algorithm: any, **kwargs):
    ...
    get_all_updates()
    print(
        f"Initializing [{algorithm.__name__()}] | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | PID: {os.getpid()}"
    )
    algorithm.init_algorithm()
    print(
        f"Starting     [{algorithm.__name__()}] | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | PID: {os.getpid()}"
    )
